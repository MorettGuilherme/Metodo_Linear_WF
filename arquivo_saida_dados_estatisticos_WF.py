...
# Objetivo do código: geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação da amplitude, fase ou pedestal pelo método WF.
...
1) Função para o cálculo dos dados estatísticos do erro de estimação pelo método Wiener Filtering (WF).
...
2) Instrução para salvar os dados estatísticos do erro de estimação da amplitude, fase ou pedestal para determinada ocupação em um arquivo de saída pelo método Wiener Filtering (WF).
...
titulo_programa = colored("Geração de arquivos de saída baseados nos dados estatísticos dos histogramas do erro de estimação da amplitude, fase, ou pedestal pelo método Wiener Filtering (WF):\n", attrs=["bold"])
...
### --- 2) INSTRUÇÃO PARA A IMPRESSÃO DOS DADOS ESTATÍSTICOS DO ERRO DE ESTIMAÇÃO DA AMPLITUDE, FASE OU PEDESTAL EM UM ARQUIVO DE SAÍDA PELO MÉTODO WF --- ###

# Definição da instrução para a impressão em um arquivo de saída, os dados estatísticos do erro de estimação do parâmetro pelo método WF.
def arquivo_saida_dados_estatisticos_erro_estimacao_parametro_WF(parametro, n_ocupacao, n_janelamento, media_erro_estimacao_parametro, var_erro_estimacao_parametro, desvio_padrao_erro_estimacao_parametro):
    ...
    pasta_saida = f"Dados_Estatisticos_WF_{parametro}_OC"
    ...
    arquivo_saida = f"dados_estatisticos_WF_{parametro}_janelamento_{n_janelamento}.txt"
...
def principal_arquivo_saida_dados_estatisticos_WF():
    ...
    for n_janelamento in tqdm(range(n_janelamento_inicial, n_janelamento_final+1, incremento_janelamento)):
        ...
        for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
            ...
            lista_erro_estimacao_amplitude = metodo_WF(Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_amplitude_referencia_treino_janelado, vetor_amplitude_referencia_teste_janelado)
            lista_erro_estimacao_fase = metodo_WF(Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_fase_referencia_treino_janelado, vetor_fase_referencia_teste_janelado)
            lista_erro_estimacao_pedestal = metodo_WF(Matriz_Pulsos_Sinais_Treino_Janelado, Matriz_Pulsos_Sinais_Teste_Janelado, vetor_pedestal_referencia_treino_janelado, vetor_pedestal_referencia_teste_janelado)
            
            media_erro_estimacao_amplitude, var_erro_estimacao_amplitude, desvio_padrao_erro_estimacao_amplitude = dados_estatisticos_erro_estimacao_parametro_WF(lista_erro_estimacao_amplitude)
            media_erro_estimacao_fase, var_erro_estimacao_fase, desvio_padrao_erro_estimacao_fase = dados_estatisticos_erro_estimacao_parametro_WF(lista_erro_estimacao_fase)
            media_erro_estimacao_pedestal, var_erro_estimacao_pedestal, desvio_padrao_erro_estimacao_pedestal = dados_estatisticos_erro_estimacao_parametro_WF(lista_erro_estimacao_pedestal)
    
            arquivo_saida_dados_estatisticos_erro_estimacao_parametro_WF(parametro_amplitude, n_ocupacao, n_janelamento, media_erro_estimacao_amplitude, var_erro_estimacao_amplitude, desvio_padrao_erro_estimacao_amplitude)
            arquivo_saida_dados_estatisticos_erro_estimacao_parametro_WF(parametro_fase, n_ocupacao, n_janelamento, media_erro_estimacao_fase, var_erro_estimacao_fase, desvio_padrao_erro_estimacao_fase)
            arquivo_saida_dados_estatisticos_erro_estimacao_parametro_WF(parametro_pedestal, n_ocupacao, n_janelamento, media_erro_estimacao_pedestal, var_erro_estimacao_pedestal, desvio_padrao_erro_estimacao_pedestal)

# Chamada da instrução principal do código.
principal_arquivo_saida_dados_estatisticos_WF()
