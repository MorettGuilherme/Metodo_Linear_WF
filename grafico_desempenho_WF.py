...
# Objetivo do código: gráfico do desempenho (EME, MSE, MAE, SNR E DP) ao longo das ocupações de acordo com o janelamento ideal para o método WF para a estimação da amplitude, fase ou pedestal.
...
1) Função para a leitura dos dados do desempenho do método WF de todas as ocupações para o janelamento ideal.
Entrada: parâmetro estimado, número do janelamento ideal, opção de avaliação do desempenho.
Saída: matriz com os dados de entrada organizados de acordo com a coluna (número da ocupação, média, variância e desvio padrão do desempenho do método WF).

2) Instrução para o plote do gráfico do desempenho do método WF ao longo das ocupações para o janelamento ideal.
Entrada: matriz dos dados de desempenho do método WF.
Saída: nada.
...
titulo_programa = colored("Plote do gráfico do desempenho (EME, MSE, MAE, SNR ou DP) ao longo das ocupações de acordo com o janelamento ideal para o método WF:\n", attrs=["bold"])

# Impressão do título do programa.
print(titulo_programa)

### ----------------------------- 1) FUNÇÃO PARA A LEITURA DOS DADOS ESTATÍSTICOS DO DESEMPENHO DO MÉTODO WF --------------------------------- ###

# Definição da função para a leitura dos dados estatísticos do desempenho do método WF.
def leitura_dados_estatisticos_desempenho_WF(parametro, n_janelamento_ideal, opcao_avaliacao_desempenho):
    ...
    pasta_dados_estatisticos_desempenho = f"K_Fold_{parametro}_{opcao_avaliacao_desempenho}_Desempenho_WF_OC"

    # Nome do arquivo de entrada dos dados estatísticos do desempenho de acordo com o janelamento ideal.
    arquivo_dados_estatisticos_desempenho = f"k_fold_{parametro}_{opcao_avaliacao_desempenho}_desempenho_WF_J_{n_janelamento_ideal}.txt"
...
### --- 2) INSTRUÇÃO PARA O PLOTE DO GRÁFICO DO DADO ESTATÍSTICO DO DESEMPENHO AO LONGO DAS OCUPAÇÕES PARA O JANELAMENTO IDEAL PELO MÉTODO WF -- ###

# Definição da instrução para o plote do gráfico do dado estatístico do desempenho ao longo das ocupações para o janelamento ideal pelo método WF.
def grafico_dado_estatistico_desempenho_WF(parametro, opcao_avaliacao_desempenho, Matriz_Dados_Desempenho):
...
def principal_grafico_dado_estatistico_desempenho_WF():
    ...
    Matriz_Dados_Desempenho = leitura_dados_estatisticos_desempenho_WF(parametro, n_janelamento_ideal, mecanismo_desempenho)
    grafico_dado_estatistico_desempenho_WF(parametro, opcao_avaliacao_desempenho, Matriz_Dados_Desempenho)
    
# Chamada da instrução principal do código.
principal_grafico_dado_estatistico_desempenho_WF()
