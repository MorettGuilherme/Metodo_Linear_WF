...
# Objetivo do código: implementação da validação cruzada K-Fold para o método WF para a estimação da amplitude, fase ou pedestal.
...
Entrada: parâmetro, número de ocupação, número de janelamento, matriz de pulsos de sinais janelada, vetor do parâmetro de referência.
Saída: nada.
...
titulo_programa = colored("Geração de arquivos de saída pela técnica de validação cruzada K-Fold para o método Filtro de Wiener (Wiener Filtering - WF):\n", attrs=["bold"])
...
# Definição da instrução para salvar as médias dos dados estatísticos da validação cruzada K-Fold em arquivo de saída para o método WF.
def arquivo_saida_dados_estatisticos_k_fold_erro_WF(parametro, n_ocupacao, n_janelamento, media_dado_erro_estimacao, var_dado_erro_estimacao, DP_dado_erro_estimacao, dado):
    ...
    pasta_saida = f"K_Fold_{parametro}_{dado}_Dados_Estatisticos_WF_OC"
    ...
    arquivo_saida = f"k_fold_{parametro}_{dado}_dados_estatisticos_WF_OC_{n_ocupacao}.txt"
...
### -------------------------------- 2) INSTRUÇÃO PARA A VALIDAÇÃO CRUZADA K-FOLD PELO MÉTODO WF --------------------------------------------- ###

# Definição da instrução da técnica de validação cruzada K-Fold para o método WF.
def K_fold_WF(parametro, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Janelado, vetor_parametro_referencia_janelado):
    ...
    for indice_teste in range(0, len(blocos_pulsos_sinais)):
        ...
        # A variável bloco_lista_erro_estimacao_parametro recebe o valor de retorno da função metodo_WF.
        Bloco_lista_erro_estimacao_parametro = metodo_WF(bloco_treino_pulsos_sinais, bloco_teste_pulsos_sinais, bloco_treino_parametro_referencia, bloco_teste_parametro_referencia)
    ...
    arquivo_saida_dados_estatisticos_k_fold_erro_WF(parametro, n_ocupacao, n_janelamento, media_media_blocos_erro_estimacao_parametro, var_media_blocos_erro_estimacao_parametro, DP_media_blocos_erro_estimacao_parametro, dado = "media")
    ...
    arquivo_saida_dados_estatisticos_k_fold_erro_WF(parametro, n_ocupacao, n_janelamento, media_var_blocos_erro_estimacao_parametro, var_var_blocos_erro_estimacao_parametro, DP_var_blocos_erro_estimacao_parametro, dado = "var")
    ...
    arquivo_saida_dados_estatisticos_k_fold_erro_WF(parametro, n_ocupacao, n_janelamento, media_DP_blocos_erro_estimacao_parametro, var_DP_blocos_erro_estimacao_parametro, DP_DP_blocos_erro_estimacao_parametro, dado = "DP")
...
def principal_K_fold_WF():
    ...
    for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
        ...
        for n_janelamento in tqdm(range(n_janelamento_inicial, n_janelamento_final+1, incremento_janelamento)):
            ...
            K_fold_WF(parametro_amplitude, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Amplitude_Janelado, vetor_amplitude_referencia_janelado)
            
            K_fold_WF(parametro_fase, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Fase_Janelado, vetor_fase_referencia_janelado)
            
            K_fold_WF(parametro_pedestal, n_ocupacao, n_janelamento, Matriz_Pulsos_Sinais_Pedestal_Janelado, vetor_pedestal_referencia_janelado)
